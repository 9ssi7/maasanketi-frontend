...
func main() {
	...
	if *useDefault {
		...
		survey = entity.DefaultSurvey()
		survey.Title = *title
		survey.Description = *description
		survey.MinCompletionTimeMin = *minTime
	} else {
		...
		if err := json.Unmarshal(jsonData, survey); err != nil {
		...
		if survey.MinCompletionTimeMin <= 0 {
			survey.MinCompletionTimeMin = 3
		}
		...
		for i, q := range survey.Questions {
			switch q.Type {
			case entity.QuestionTypeText, entity.QuestionTypeNumber, entity.QuestionTypeSelect, entity.QuestionTypeMultiSelect, entity.QuestionTypeBoolean:
				// Valid type
			default:
			...
			if (q.Type == entity.QuestionTypeSelect || q.Type == entity.QuestionTypeMultiSelect) && len(q.Options) == 0 {
